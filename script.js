...
const commands={
  help:"about  skills  projects  contact  theme  clear  sudo make me hire  neofetch",
  about:"Creative frontend developer focused on interactive interfaces and web apps.",
  skills:"JavaScript ██████████\nCSS ██████████\nReact █████████",
...
input.addEventListener("keydown", e=>{
  ...
  if(e.key==="ArrowUp"){histIndex=Math.max(0,histIndex-1); input.value=history[histIndex]||"";}
  if(e.key==="ArrowDown"){histIndex=Math.min(history.length,histIndex+1); input.value=history[histIndex]||"";}
  if(e.key==="Tab"){
    e.preventDefault();
...
function run(cmd){
  switch(cmd){
    ...
    case "theme": themeIndex=(themeIndex+1)%themes.length; applyTheme(themes[themeIndex]); print("Theme switched ✔"); break;
    case "sudo make me hire": print("Access granted 😎"); break;
    case "neofetch":
      print("<strong>Tu Nombre</strong> Portfolio OS");
      print("OS: Web Terminal");
      print("Shell: JS Custom");
      print("Terminal Portfolio: Interactive Web Developer CV");
      break;
    default:
